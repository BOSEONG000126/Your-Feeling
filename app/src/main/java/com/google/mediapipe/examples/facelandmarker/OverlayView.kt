...
class OverlayView(context: Context?, attrs: AttributeSet?) :
    ...
    override fun draw(canvas: Canvas) {
        ...
        results?.let { faceLandmarkerResult ->
            ...
            //var bobo: Int = 1
            //val nullableArrayx = arrayOfNulls<Float>(500)
            //val nullableArrayy = arrayOfNulls<Float>(500)
            for(landmark in faceLandmarkerResult.faceLandmarks()) {
                for(normalizedLandmark in landmark) {
                    ...
                    //Log.i(TAG,bobo.toString() +"x : " + normalizedLandmark.x().toString() + " , " + bobo.toString()+"y : " + normalizedLandmark.y().toString())
                    //nullableArrayx[bobo-1] = normalizedLandmark.x()
                    //nullableArrayy[bobo-1] = normalizedLandmark.y()
                    //bobo +=1
                }
            }
    ...
    fun getInfo(): String {
        // 필요한 정보를 계산하거나 필드 값을 반환하는 로직 작성
        val info = "OverlayView의 정보"
        return info
    }
